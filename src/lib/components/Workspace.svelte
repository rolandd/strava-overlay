<script lang="ts">
	import { ImagePlus, Layers, Hand, Move, ZoomIn } from '@lucide/svelte';
	...
	let overlayBaseWidth = $derived(
		containerWidth > 0 ? containerWidth * 0.85 : 300
	);
...
<div class="w-full max-w-4xl mx-auto px-4 py-2 flex flex-col items-center select-none">
	...
	>
		...
		{#if isDragOver}
			<div class="absolute inset-0 z-40 bg-[#09090b]/85 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center border-2 border-dashed border-[#fc4c02]">
		...
		{:else}
			<!-- Empty State -->
			<div class="w-full h-full flex flex-col items-center justify-center p-8 text-center bg-[#101014]">
				<div class="w-16 h-16 rounded-2xl bg-[#1a1a22] border border-[#27272f] flex items-center justify-center text-zinc-400 mb-4 shadow-inner">
		...
		{#if overlayItem && baseItem}
			...
			>
				...
				>
					...
					{#if isInteracting}
						<div class="absolute -top-6 left-1/2 -translate-x-1/2 bg-black/80 text-white text-[10px] font-mono px-2 py-0.5 rounded-full border border-zinc-700 whitespace-nowrap pointer-events-none">
		...
		{:else if baseItem && !overlayItem}
			<div class="absolute inset-0 pointer-events-none flex items-center justify-center">
				<div class="bg-black/70 backdrop-blur-sm border border-zinc-700/60 px-4 py-2 rounded-xl text-center shadow-lg">
		...
		{#if baseItem && overlayItem}
			<div class="absolute bottom-2 left-2 pointer-events-none z-30 opacity-70 hover:opacity-100 transition-opacity">
				<div class="bg-black/60 backdrop-blur-md px-2 py-1 rounded-md text-[10px] text-zinc-300 border border-white/10 flex items-center gap-1.5">
