<script lang="ts">
	...
	import type { AspectRatioType, BaseImageAdjustments, ExportOptions, OverlayTransform } from '../types';
	...
	function stepScale(delta: number) {
		const newScale = Math.max(0.1, Math.min(5.0, Math.round((transform.scale + delta) * 100) / 100));
	...
	<div class="bg-[#121217] rounded-2xl border border-[#27272f] overflow-hidden shadow-xl">
		...
		<div class="flex items-center justify-between border-b border-[#22222a] px-3 pt-2">
			<div class="flex items-center gap-1">
				<button
					onclick={() => { activeTab = 'transform'; isCollapsed = false; }}
					class="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-t-lg transition-all border-b-2 cursor-pointer {activeTab === 'transform' && !isCollapsed
				...
				<button
					onclick={() => { activeTab = 'base'; isCollapsed = false; }}
					class="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-t-lg transition-all border-b-2 cursor-pointer {activeTab === 'base' && !isCollapsed
						? 'text-[#fc4c02] border-[#fc4c02] bg-[#1a1a22]/60'
						: 'text-zinc-400 border-transparent hover:text-zinc-200'}"
				>
				...
				<button
					onclick={() => { activeTab = 'export'; isCollapsed = false; }}
					class="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-t-lg transition-all border-b-2 cursor-pointer {activeTab === 'export' && !isCollapsed
						? 'text-[#fc4c02] border-[#fc4c02] bg-[#1a1a22]/60'
						: 'text-zinc-400 border-transparent hover:text-zinc-200'}"
				>
			...
			<div class="p-4">
				...
				{#if activeTab === 'base'}
					<div class="space-y-4">
						...
						<!-- Crop Framing & Reset -->
						<div class="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-[#22222a]">
							<div class="flex items-center gap-1.5">
								<span class="text-xs font-medium text-zinc-400 mr-1 flex items-center gap-1">
									<Crop class="w-3.5 h-3.5" /> Aspect Crop:
								</span>
								{#each cropRatios as ratio}
									<button
										onclick={() => onUpdateAdjustments({ cropAspectRatio: ratio.id })}
										disabled={!hasBase}
										class="px-2.5 py-1 rounded-lg text-xs font-medium transition cursor-pointer {adjustments.cropAspectRatio === ratio.id
				...
				{#if activeTab === 'export'}
					<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
						...
						<div class="space-y-2">
							...
							<div class="grid grid-cols-2 gap-2">
								<button
									...
									class="py-1.5 px-3 rounded-lg text-xs font-semibold transition cursor-pointer {exportOptions.format === 'image/jpeg'
										? 'bg-[#fc4c02] text-white'
								...
								<button
									...
									class="py-1.5 px-3 rounded-lg text-xs font-semibold transition cursor-pointer {exportOptions.format === 'image/png'
										? 'bg-[#fc4c02] text-white'
										: 'bg-[#1a1a22] text-zinc-300 border border-[#27272f]'}"
								>
									PNG (Lossless)
						...
						<div class="space-y-2 {exportOptions.format === 'image/png' ? 'opacity-40 pointer-events-none' : ''}">
							<div class="flex items-center justify-between text-xs">
						...
						<div class="space-y-2">
							...
							<div class="grid grid-cols-3 gap-1">
								<button
									...
									class="py-1.5 px-2 rounded-lg text-[11px] font-semibold transition cursor-pointer {exportOptions.maxDimension === 4096
										? 'bg-[#fc4c02] text-white'
										: 'bg-[#1a1a22] text-zinc-300 border border-[#27272f]'}"
								>
									Full (4K+)
								...
								<button
									...
									class="py-1.5 px-2 rounded-lg text-[11px] font-semibold transition cursor-pointer {exportOptions.maxDimension === 2560
										? 'bg-[#fc4c02] text-white'
										: 'bg-[#1a1a22] text-zinc-300 border border-[#27272f]'}"
								>
									2.5K
								...
								<button
									...
									class="py-1.5 px-2 rounded-lg text-[11px] font-semibold transition cursor-pointer {exportOptions.maxDimension === 1080
										? 'bg-[#fc4c02] text-white'
										: 'bg-[#1a1a22] text-zinc-300 border border-[#27272f]'}"
								>
									1080p
