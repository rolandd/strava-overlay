<script lang="ts">
	import { Image as ImageIcon, Layers, ArrowLeftRight, X, Upload } from '@lucide/svelte';
...
<div class="w-full max-w-4xl mx-auto px-4 py-2">
	<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 relative">
		...
		>
			{#if baseItem}
				<div class="flex items-center gap-3 overflow-hidden">
					<div class="w-12 h-12 rounded-lg bg-black overflow-hidden shrink-0 border border-[#27272f] flex items-center justify-center">
						<img
							src={baseItem.url}
							alt="Base thumbnail"
			...
			{:else}
				...
				>
					<div class="w-10 h-10 rounded-lg bg-[#1a1a22] border border-[#27272f] flex items-center justify-center text-zinc-400 group-hover:text-[#fc4c02] group-hover:border-[#fc4c02]/40 transition-colors">
						<ImageIcon class="w-5 h-5" />
					</div>
					<div>
						<div class="text-xs font-semibold text-zinc-200 group-hover:text-white transition-colors">
							1. Select Base Photo
						</div>
		...
		>
			{#if overlayItem}
				<div class="flex items-center gap-3 overflow-hidden">
					<div class="w-12 h-12 rounded-lg bg-black/60 overflow-hidden shrink-0 border border-[#27272f] flex items-center justify-center p-1 bg-[radial-gradient(#27272f_1px,transparent_1px)] [background-size:6px_6px]">
						<img
							src={overlayItem.url}
